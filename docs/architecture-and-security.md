...
| Brand logos and colors | Every contract shows in its brand's colors with the brand's logo; brands are Leave's own table, maintained in-house | A `brands` table (keyed by brand only, never an athlete), a separate `brands` service that fetches public brand marks from the brand's own site (isolated from every key and from contract data), a private logo bucket served through a CDN at cdn.leaveyouragent.com, a task queue and a monthly refresh |
...
| Brands service | An internet fetcher sitting next to decrypt keys | The logo fetcher runs as its own service with no key roles, no access to the files bucket or secrets, database grants limited to the brands tables, and a network tag the Neo4j firewall rule does not admit; it visits brand sites under the same SSRF rules as the crawler |
...
iPhone app ──┐                                   ChatGPT / Claude / Gemini / other MCP-capable apps
   ...
   Cloud Run "api" (min 1 instance)              (same service, second host rule)
     REST + MCP resource server + Stripe and WorkOS webhooks
             ...
             ├──► Cloud Tasks "brands"     ──► Cloud Run "brands" (min 0): fetches a brand's public logo and colors from its own site; no keys, no contract data, no route to Neo4j
             ├──► cdn.leaveyouragent.com: Cloud CDN in front of the private logo bucket (public-access prevention stays enforced)
             │                                  (Document AI only for unreadable scans) → Gemini structured  
             │                                  output → Postgres/pgvector → Neo4j → brands table
