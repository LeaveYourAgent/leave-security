...
`deviceApproved` is true for the first device on a new account and false for any later device until section 4 completes. Errors: `invalid_code` (400; five wrong attempts invalidate the code), `code_expired` (400, after 10 minutes), `rate_limited` (429 with `retryAfter`). `device` may be omitted (v1.3): the token is then device-less, `deviceId` is null, and it may call only the device-less routes listed in section 4; the first device registered on an account that has no approved device and no account public key yet is approved automatically (an account that once had devices and lost them all goes through recovery instead). `GET /account` answers `profile_required` (403) until the profile has been created. No private data can exist on an account until it has an approved device.
